...
+ (void)setCenterCoordinate:(MKMapView *)mapToCenter centerCoordinate:(CLLocationCoordinate2D)centerCoordinate zoomLevel:(NSUInteger)zoomLevel animated:(BOOL)animated
{
    ...
    MKCoordinateSpan span = [__CLASS_PREFIX__MapHelper coordinateSpanWithMapView:mapToCenter centerCoordinate:centerCoordinate zoomLevel:zoomLevel];
...
+ (MKCoordinateSpan)coordinateSpanWithMapView:(MKMapView *)mapView centerCoordinate:(CLLocationCoordinate2D)centerCoordinate zoomLevel:(NSUInteger)zoomLevel
{
    ...
    double centerPixelX = [__CLASS_PREFIX__MapHelper longitudeToPixelSpaceX:centerCoordinate.longitude];
    double centerPixelY = [__CLASS_PREFIX__MapHelper latitudeToPixelSpaceY:centerCoordinate.latitude];
    ...
    double minLng = [__CLASS_PREFIX__MapHelper pixelSpaceXToLongitude:topLeftPixelX];
    double maxLng = [__CLASS_PREFIX__MapHelper pixelSpaceXToLongitude:topLeftPixelX + scaledMapWidth];
    double longitudeDelta = maxLng - minLng;
    
    // find delta between top and bottom latitudes
    double minLat = [__CLASS_PREFIX__MapHelper pixelSpaceYToLatitude:topLeftPixelY];
    double maxLat = [__CLASS_PREFIX__MapHelper pixelSpaceYToLatitude:topLeftPixelY + scaledMapHeight];
